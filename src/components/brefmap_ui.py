...
def render_pdf_panel(statement_type: str, result: dict, key_prefix: str = ""):
    ...
    if BREF_MAPPING_AVAILABLE and rows:
        ...
        with col_template:
            ...
            # Load region-specific template (will be set after region selection)
            template_bytes = load_bref_template(bref_region)
            template_name = "KLN.xlsx" if bref_region == "APAC" else "NEXTERA_4.xlsx"
            ...
            if template_bytes:
                ...
                st.warning(f"⚠️ {template_name} not found in project root")
        ...
        # Only display results for the CURRENT statement type to avoid duplicate keys
        # Each statement tab will show its own results
...
def _display_mapping_results(mapping_key: str, statement_type: str, key_prefix: str):
    ...
    with col_clear:
        ...
        clear_key = f"clear_{mapping_key.replace('_', '-')}"
