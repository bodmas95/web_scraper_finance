...
def should_negate_field(
    field_label: str,
    statement_type: str = "income_statement",
    region: str = "US"
) -> bool:
    """
    Check if a field should have its sign negated (multiplied by -1).
    
    This is used when extracting values from source data that may have
    opposite signs from what BREF expects.
    
    Args:
        field_label: Field label (e.g., "I4 | SG&A Expense")
        statement_type: "income_statement", "balance_sheet", or "cash_flow"
        region: "US" (AMER), "APAC", or "EMEA"
    
    Returns:
        True if field should be negated, False otherwise
    """
    ...
    # If field is in force_positive list, it should be negated if currently negative
    return clean_label in force_positive_fields
