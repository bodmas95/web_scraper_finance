...
def _run_hkex():
    ...
    def validate_arguments(args):
        ...
        if not company.isdigit() or len(company) != 5:
            raise ValueError(f"Invalid company format: {company}. Expected 5-digit HKEX code (e.g., 01929)")
        ...
        if using_range:
            ...
            try:
                ...
                if year_from < 2000 or year_from > current_year + 1:
                    raise ValueError(f"Invalid year_from: {year_from}. Must be between 2000 and {current_year + 1}")
                
                if year_to < 2000 or year_to > current_year + 1:
                    raise ValueError(f"Invalid year_to: {year_to}. Must be between 2000 and {current_year + 1}")
                
                if year_from > year_to:
                    raise ValueError(f"year_from ({year_from}) cannot be greater than year_to ({year_to})")
                
                if year_to - year_from > 9:  # 10 years max
                    raise ValueError(f"Year range too large ({year_to - year_from + 1} years). Maximum allowed is 10 years.")
                    
            except ValueError as e:
                if "invalid literal" in str(e):
                    raise ValueError(f"Invalid year format in range: {args.get('year_from', '')} or {args.get('year_to', '')}")
                raise
        else:
            ...
            try:
                ...
                if year_int < 2000 or year_int > current_year + 1:
                    raise ValueError(f"Invalid year: {year_int}. Must be between 2000 and {current_year + 1}")
            except ValueError as e:
                if "invalid literal" in str(e):
        ...
        if args['report_type'].lower() not in valid_report_types:
            raise ValueError(f"Invalid report_type: {args['report_type']}. Must be one of: {', '.join(valid_report_types)}")
    ...
    def run_pipeline(company: str, source: str, report_type: str, year: str) -> tuple:
        ...
        logger.info(f"Starting HKEX pipeline with parameters:")
        ...
        try:
            ...
            if not company_doc:
                raise ValueError(f"Company not found for symbol: {company}. Please ensure the company exists in the database.")
            ...
            if not source_doc:
                raise ValueError(f"Source '{source}' not found for company: {company_doc['name']}. Please check source configuration.")
            ...
            print(f"Fetching data from HKEX...")
            ...
            print(f"Parsing reports from HTML...")
    ...
    def run_pipeline_for_years(company: str, source: str, report_type: str, years: list) -> None:
        ...
        print(f"HKEX Data Ingestion Pipeline - Multi-Year")
        print("=" * 60)
        ...
        # Final summary
        print(f"Multi-Year Pipeline Completed!")
        print("=" * 60)
